...
final _periodoValido = DateTime(2020, 1, 1);

void main() {
  group('Testes de Valores-Limite', () {
    group('CT-BV01 (RN01): Nome do estudante no limite vazio', () {
      test('Dado nome vazio Quando criar estudante Então construtor rejeita', () {
        expect(
          () => Estudante(nome: '', numeroMatricula: '2024001', situacaoAtiva: true, possuiPendencia: false),
          throwsArgumentError,
        );
      });
    });
    ...
    group('CT-BV04 (RN04): Nome da disciplina no limite vazio', () {
      test('Dado nome da disciplina vazio Quando criar disciplina Então construtor rejeita', () {
        expect(
          () => Disciplina(nome: '', vagasMaximas: 40, vagasPreenchidas: 0, horario: 'SEG 08:00'),
          throwsArgumentError,
        );
      });
    });
    ...
    group('CT-BV05 (RN05): Vagas máximas no limite zero', () {
      test('Dado vagasMaximas = 0 Quando criar disciplina Então construtor rejeita', () {
        expect(
          () => Disciplina(nome: 'Matemática', vagasMaximas: 0, vagasPreenchidas: 0, horario: 'SEG 08:00'),
          throwsArgumentError,
        );
      });
    });
  ...
  group('Critérios de Aceitação', () {
    group('CA-01: Matrícula bem-sucedida de estudante válido', () {
      test('Dado estudante ativo sem pendências e disciplina com vaga Quando realizar matrícula Então vagas aumentam', () {
        final sistema = SistemaMatriculas();
        sistema.configurarPeriodoMatricula(DateTime(2020, 1, 1), DateTime(2030, 12, 31));
        final e = Estudante(nome: 'João', numeroMatricula: '2024001', situacaoAtiva: true, possuiPendencia: false);
    ...
    group('CA-03 (RN06): Matrícula rejeitada por disciplina lotada', () {
      test('Dado disciplina lotada Quando realizar matrícula Então sistema rejeita', () {
        final sistema = SistemaMatriculas();
        sistema.configurarPeriodoMatricula(DateTime(2020, 1, 1), DateTime(2030, 12, 31));
        final e = Estudante(nome: 'João', numeroMatricula: '2024001', situacaoAtiva: true, possuiPendencia: false);
        final d = Disciplina(nome: 'Matemática', vagasMaximas: 40, vagasPreenchidas: 40, horario: 'SEG 08:00');
    ...
    group('CA-04 (RN11): Matrícula rejeitada fora do período', () {
      test('Dado período no passado Quando realizar matrícula Então sistema rejeita', () {
        final sistema = SistemaMatriculas();
        sistema.configurarPeriodoMatricula(DateTime(2020, 1, 1), DateTime(2020, 1, 31));
        final e = Estudante(nome: 'João', numeroMatricula: '2024001', situacaoAtiva: true, possuiPendencia: false);
        expect(() => sistema.realizarMatricula(e, []), throwsArgumentError);
      });
    });
    ...
    group('CA-05 (RN12): Vagas atualizadas após confirmação', () {
      test('Dado matrícula confirmada Quando realizar matrícula Então vagasPreenchidas incrementa', () {
        final sistema = SistemaMatriculas();
        sistema.configurarPeriodoMatricula(DateTime(2020, 1, 1), DateTime(2030, 12, 31));
        final e = Estudante(nome: 'João', numeroMatricula: '2024001', situacaoAtiva: true, possuiPendencia: false);
        final d = Disciplina(nome: 'Matemática', vagasMaximas: 40, vagasPreenchidas: 10, horario: 'SEG 08:00');
